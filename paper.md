...
Such initial configuration is dynamically evolved through a numerical simulation and the default output is written into `T\_Sun.dat` every 15 minutes. 
...
Performing such task of plotting the mean orbits without the library, would demand the use of the additional file `K\_Sun.dat`. 
...
Triangles and circles stand for the osculating elements from `K\_Sun.dat`, but the lines in peach color were evaluated from the default output file `T\_Sun.dat` via `liborbital` library. 

![Circles and triangles stand for the osculating elements from `K\_Sun.dat` while the peach color curves were evaluated from `T\_Sun.dat` by using the `liborbital` functions. \label{fig:ex1}](ex1.pdf){widht=70%}
...
Analogously to the figure \autoref{fig:ex1}, the circles stand for data from file `K\_Earth.dat` while the back curves in peach color were plotted directly from `T\_Sun.dat` with `liborbital` functions. 
